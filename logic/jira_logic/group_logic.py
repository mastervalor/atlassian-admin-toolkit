...
class Groups:
    ...
    def get_group_users_email(self, group):
        ...
        members_list = []
        try:
            self.jira_groups.get_group(f'?startAt={start_at}&maxResults={max_results}', group)
            while total >= max_results:
                response = self.jira_groups.get_group(f'?startAt={start_at}&maxResults={max_results}', group)
